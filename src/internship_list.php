...
<main class="main">
  ...
  <div class="stats-row">
    ...
    <div class="stat-card">
      ...
      <div class="stat-value purple" id="stat-unassigned">0</div>
    </div>
  </div>
...
<script>
  ...
  function loadInternships() {
    fetch('get_internships.php')
      ...
      .then(result => {
        ...
        document.getElementById("stat-total").textContent = result.counts?.total ?? data.length;
        document.getElementById("stat-completed").textContent = result.counts?.completed ?? data.filter(r => r.status === "completed").length;
  ...
  function updateStats() {
    ...
    document.getElementById("stat-unassigned").textContent = data.filter(r => r.status === "unassigned").length;
  }
  ...
  function render() {
    ...
    const end = currentPage * ROWS_PER_PAGE;
        ...
        <tr>
          ...
          <td>
            <div class="actions">
              ${
                r.status === "unassigned"
                  ? `<button class="icon-btn" title="Assign" onclick="assignRecord('${r.student_id}')">
                      <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path d="M12 5v14"/>
                        <path d="M5 12h14"/>
                      </svg>
                    </button>`
                  : `<button class="icon-btn" title="Edit" onclick="editRecord('${r.internship_id}')">
                      <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    document.getElementById("pageInfo").textContent =
      `Showing ${total === 0 ? 0 : start + 1}-${Math.min(end, total)} of ${total} records`;
  ...
  document.getElementById("programmeFilter").addEventListener("change", () => {
    currentPage = 1;
    render();
  });
