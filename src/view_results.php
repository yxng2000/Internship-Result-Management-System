...
$sql = "
    SELECT
        ...
        a.undertaking_tasks        AS task_score,
        a.health_safety            AS safety_score,
        a.theoretical_knowledge    AS knowledge_score,
        a.report_presentation      AS report_score,
        a.clarity_language         AS language_score,
        a.lifelong_learning        AS lifelong_score,
        a.project_management       AS project_score,
        a.time_management          AS time_score,
        a.total_score,
...
<style>
  :root {
    --navy:   #0f1b2d; --ink: #1a2e45; --teal: #0d7377;
    --gold:   #e8a838; --cream: #f7f3ec; --white: #ffffff;
    --muted:  #6b7a8d; --border: #d8dce3; --shadow: 0 4px 24px rgba(15,27,45,0.10);
  }
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .topbar-nav a { color: rgba(255,255,255,0.7); text-decoration: none; font-size: 0.85rem; padding: 0.4rem 0.9rem; border-radius: 6px; transition: all 0.2s; }
  ...
  .stat-card { background: var(--white); border-radius: 12px; padding: 1.2rem 1.5rem; box-shadow: var(--shadow); display: flex; align-items: center; gap: 1rem; }
  ...
  .search-bar { background: var(--white); border-radius: 12px; padding: 1.2rem 1.5rem; box-shadow: var(--shadow); margin-bottom: 1.5rem; display: flex; gap: 0.8rem; align-items: flex-end; flex-wrap: wrap; }
  ...
  .search-group input, .search-group select { padding: 0.65rem 1rem; border: 1.5px solid var(--border); border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; color: var(--navy); background: var(--cream); transition: border-color 0.2s; }
  .search-group input:focus, .search-group select:focus { outline: none; border-color: var(--teal); background: var(--white); }
  .btn-search { padding: 0.65rem 1.5rem; background: var(--teal); color: var(--white); border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; font-weight: 600; cursor: pointer; transition: background 0.2s; white-space: nowrap; }
  .btn-search:hover { background: #0a5e62; }
  .btn-clear { padding: 0.65rem 1rem; background: transparent; color: var(--muted); border: 1.5px solid var(--border); border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; cursor: pointer; transition: all 0.2s; }
  ...
  thead th { padding: 1rem 1.2rem; text-align: left; font-size: 0.75rem; font-weight: 600; letter-spacing: 0.07em; text-transform: uppercase; color: rgba(255,255,255,0.7); white-space: nowrap; }
  ...
  .grade-badge  { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; color: var(--white); }
  ...
  .score-breakdown { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.5rem 1rem; margin-bottom: 0.8rem; }
  .breakdown-item { font-size: 0.82rem; }
  .breakdown-label { color: var(--muted); }
  .breakdown-value { font-weight: 600; color: var(--navy); }
  .comment-box { background: var(--white); border-left: 3px solid var(--teal); padding: 0.7rem 1rem; border-radius: 0 8px 8px 0; font-size: 0.85rem; color: var(--navy); font-style: italic; }
  .toggle-btn { background: none; border: none; cursor: pointer; color: var(--teal); font-size: 0.8rem; font-weight: 600; padding: 4px 10px; border-radius: 6px; transition: background 0.15s; }
...
<div class="page-wrap">
  ...
  $avg_score  = $total_assessed > 0 ? round(array_sum(array_column($results, 'total_score')) / $total_assessed, 2) : 0;
  $high_score = $total_assessed > 0 ? max(array_column($results, 'total_score')) : 0;
  ?>
  <div class="stats-row">
    <div class="stat-card"><div class="stat-icon">🎓</div><div><div class="stat-label">Students Assessed</div><div class="stat-value"><?= $total_assessed ?></div></div></div>
    <div class="stat-card"><div class="stat-icon">📊</div><div><div class="stat-label">Average Score</div><div class="stat-value"><?= $avg_score ?></div></div></div>
    <div class="stat-card"><div class="stat-icon">🏆</div><div><div class="stat-label">Highest Score</div><div class="stat-value"><?= $high_score ?></div></div></div>
  </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="total-cell">
              <?= number_format($r['total_score'], 2) ?><br>
              <span class="grade-badge" style="background:<?= $grade_color ?>"><?= $grade ?></span>
            </td>
            <td><button class="toggle-btn" onclick="toggleDetail(<?= $i ?>)">Details ▾</button></td>
          </tr>
